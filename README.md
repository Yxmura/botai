# botAI
A host-it-yourself AI Discord Bot maker. Just tell it your ideas and it makes them real.

# How to use?
1.Download the Code as a .zip file. <br>
2. Unzip it. <br>
3. Edit .env and fill either a Gemini (AI Studio) API Key or an OpenAI API Key. <br>
4. Run "pip install requirements.txt" <br>
5. Do "python app.py" or "python3 app.py". <br>
6. You can visit the AI Bot Maker with any browser at URL: localhost:5500
